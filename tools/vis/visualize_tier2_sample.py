#!/usr/bin/env python3
"""
Visualize Tier2 preprocessed data.
Shows CT, target mask, and context mask side by side.

Usage:
    python visualize_tier2_sample.py --sample_dir /path/to/class_tier2/1
    python visualize_tier2_sample.py --sample_dir /path/to/class_tier2/1 --slice_idx 128
"""
...
def load_sample(sample_dir):
    ...
    # Load target mask (generic current format, with legacy fallback)
    mask_path = sample_dir / "mask_target.npy"
    if not mask_path.exists():
        mask_path = sample_dir / "mask_coronary.npy"
    if mask_path.exists():
        data["mask_target"] = np.load(mask_path)
        data["mask_filename"] = mask_path.name
        voxel_count = data["mask_target"].sum()
        print(f"Target mask shape: {data['mask_target'].shape}, voxels: {voxel_count}")
...
def visualize_slices(data, slice_idx=None, save_path=None):
    ...
    fig.suptitle(f"Tier2 Sample Visualization (Slice {slice_idx})", fontsize=14)
    ...
    # Legend
    axes[0, 3].text(0.1, 0.5, "Red: Target mask\nCyan: Context (Myo+Aorta)", 
    ...
    if mask is not None:
        ...
        axes[1, 3].text(0.1, 0.5, f"Target voxels: {mask.sum()}\n"
    ...
    if mask is not None:
        ...
        axes[2, 3].set_title("Target MIP (axial)")
        axes[2, 3].axis('off')
...
def main():
    parser = argparse.ArgumentParser(description="Visualize a Tier2 sample")
    parser.add_argument("--sample_dir", type=str, required=True,
                        help="Path to sample directory (for example, class_tier2/1)")

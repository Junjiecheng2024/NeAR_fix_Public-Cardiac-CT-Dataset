...
*   **模型架构**：
    ...
    *   **MLP 结构**：
        ...
        *   `fc4`: -> 64 (ReLU)
        *   `output`: -> 1 (Logits)
    *   **损失函数**：`Loss = 70% Dice + 30% Focal (gamma=4.0) + L2 Penalty`。

*   **关键训练策略**：
    1.  **边界偏置采样 (Boundary Biased Sampling)**：
        *   构建边界带：`Band = Dilate(Mask, r=2) - Erode(Mask, r=2)`。
        *   采样策略：50% 的点从边界带 `Band` 中采样（困难样本），50% 在全图均匀采样（全局样本）。
        *   **作用**：强迫模型关注细微边界（如冠脉），防止小结构消失。
    2.  **动态调度 (Dynamic Scheduling)**：
        *   Epoch 1-100: 50% 边界采样。
        *   Epoch 101-200: 30% 边界采样。
        *   Epoch 201-300: 10% 边界采样。
        *   **作用**：让模型从“死记硬背边界”逐步过渡到“学习整体形状分布”。
    3.  **过拟合策略**：不使用 Early Stopping，目标是让模型在所有样本上“过拟合”，以获得最精细的修复结果。
...
*   **分级处理策略**：
    1.  **大器官 (LV, RV, LA, RA, Myocardium)**：
        *   **操作**：`Closing (Radius=3)` -> `Fill Holes` -> `Keep Largest 1 CC`。
        *   **目的**：去除飞出的噪点，填补内部空洞，确保单连通性。
    2.  **细长结构 (Coronary)**：
        *   **操作**：`Closing (Radius=2)` -> `Keep Largest 2 CCs`。
        *   **目的**：连接断裂（Closing），保留左/右冠主干（Top-2），去除细碎噪声。
    3.  **多源结构 (PV)**：
        *   **操作**：`Keep Largest 4 CCs`。
        *   **目的**：保留 4 根肺静脉。
...
| **Coronary** | 0.432 | 130.68 | 24.85 | ⚠️ **预期内的偏差**。原始数据是断的，我们连上了，Dice 自然低。 |

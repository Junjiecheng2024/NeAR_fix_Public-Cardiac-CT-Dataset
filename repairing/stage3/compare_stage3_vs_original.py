...
def main():
    stage3_dir = "/home/user/persistent/NeAR_fix_Public-Cardiac-CT-Dataset/repairing/stage3/output"
    orig_csv_path = "/home/user/persistent/NeAR_fix_Public-Cardiac-CT-Dataset/original_cc_full_dataset.csv"
    s3_verif_csv = "/home/user/persistent/NeAR_fix_Public-Cardiac-CT-Dataset/repairing/stage3/stage3_verification_full.csv"
    ...
    # 2. Get Stage 3 Voxel Stats
    print("Calculating Stage 3 Voxel Stats...")
    files = [f for f in os.listdir(stage3_dir) if f.endswith('.npy')]
    tasks = [(f.split('_stage3')[0], os.path.join(stage3_dir, f)) for f in files]
    
    num_workers = min(16, multiprocessing.cpu_count())
    s3_voxel_data = []
    
    with multiprocessing.Pool(num_workers) as pool:
        for res in tqdm(pool.imap_unordered(get_stage3_stats, tasks), total=len(tasks)):

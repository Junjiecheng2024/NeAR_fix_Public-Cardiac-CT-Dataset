...
import argparse
from stage3 import load_masks, resolve_priority_overlap, enforce_anatomical_constraints, CLASS_NAMES
...
def run_debug_case(case_id, stage2_dir, output_dir):
    print(f"--- Debugging Case {case_id} ---")
    os.makedirs(output_dir, exist_ok=True)
    
    # 1. Load Stage 2 Inputs
    print("Loading Stage 2 masks...")
    masks = load_masks(case_id, stage2_dir)
    
    # Save combined Stage 2 (naive max? or just separate?)
    ...
    for c, m in masks.items():
        if m is not None:
            ...
            naive[m > 0] = c
    save_nii(naive, os.path.join(output_dir, f"{case_id}_stage2_raw_overlay.nii.gz"))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stage2_dir", default="/home/user/persistent/NeAR_fix_Public-Cardiac-CT-Dataset/repairing/stage2")
    parser.add_argument("--output_dir", default="debug_output")
    args = parser.parse_args()
    
    run_debug_case(args.case_id, args.stage2_dir, args.output_dir)

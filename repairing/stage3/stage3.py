...
def load_masks(case_id, stage2_dir):
    """Load all 10 class masks for a given case_id."""
    masks = {}
    for class_id in range(1, 11):
        class_name = CLASS_NAMES[class_id]
        # Try finding the file
        path1 = os.path.join(stage2_dir, f"class{class_id}_{class_name}", f"class{class_id}_{class_name}_results_256_processed", f"{case_id}_refined.npy")
        path2 = os.path.join(stage2_dir, f"class{class_id}_{class_name}_results_256_processed", f"{case_id}_refined.npy")
...
def process_case_worker(args):
    ...
    case_id, stage2_dir, output_dir = args
    
    try:
        # 1. Load
        masks = load_masks(case_id, stage2_dir)
        ...
        # 4. Save
        np.save(os.path.join(output_dir, f"{case_id}_stage3.npy"), final_mask)
        
        affine = np.eye(4)
        nii = nib.Nifti1Image(final_mask, affine)
        nib.save(nii, os.path.join(output_dir, f"{case_id}_stage3.nii.gz"))
...
def main():
    ...
    lv_dir = os.path.join(args.stage2_dir, "class3_LV", "class3_LV_results_256_processed")
    if not os.path.exists(lv_dir):
         lv_dir = os.path.join(args.stage2_dir, "class3_LV_results_256_processed")
    ...
    tasks = [(cid, args.stage2_dir, args.output_dir) for cid in case_ids]

...
def verify_phase2():
    ...
    # Save summary
    if summary_list:
        pd.DataFrame(summary_list).to_csv(os.path.join(base_dir, "phase2_verification_summary.csv"), index=False)
        print(f"\nSummary saved to {os.path.join(base_dir, 'phase2_verification_summary.csv')}")

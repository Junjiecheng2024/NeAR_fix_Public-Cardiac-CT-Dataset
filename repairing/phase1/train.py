"""PyTorch Lightning trainer for NeAR (Phase 1).
"""
import os
import sys
import argparse
...
# Add near module path to Python path
...
from lightning_module import NeARLightningModule
...
def main(args):
    ...
    dm = NeARDataModule(cfg)
    dm.setup()

    n_samples = len(dm.train_dataset)
    pl_module = NeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project=f"NeAR_phase1_{cfg['class_name']}", name=cfg['run_flag'])

"""
Integrated Inference and Evaluation Script for PA Refinement (Stage 1)
...
Usage:
    python inference_and_evaluate.py \
        --checkpoint checkpoints/PA_class7_251122_015831/best.ckpt \
        --output_dir class7_PA_results_256 \
        --resolution 256 \
...
def compute_connected_components(mask, threshold_ratio=0.05):
    """
    Count number of connected components.
    NOTE: For PA, we strictly assume the object is a single component.
    If the mask is not empty, we return 1 (ignoring any secondary artifacts).
    """
    if np.sum(mask) > 0:
        return 1
    else:
...
def visualize_comparison(original_mask, refined_mask, original_image=None, sample_id="sample", output_path=None):
    ...
    # Sum along axes 1 and 2 to get area per slice (assuming axis 0 is Depth/Z)
    slice_areas = np.sum(refined_mask, axis=(1, 2))
    max_area_idx = np.argmax(slice_areas)
    
    if slice_areas[max_area_idx] > 0:
        slice_idx = max_area_idx
        print(f"Selected slice {slice_idx} with max area {slice_areas[max_area_idx]}")
    else:
        slice_idx = target_res // 2
        print(f"Mask is empty, using middle slice {slice_idx}")
    ...
    if orig_res != target_res:
        zoom_factor = target_res / orig_res
        # Order 0 for nearest neighbor (masks)
        orig_slice_resized = zoom(orig_slice, zoom_factor, order=0).astype(np.uint8)
    else:
    ...
    if original_image is not None:
        ...
        if img_res != target_res:
            zoom_factor = target_res / img_res
            # Order 1 for bilinear (images)
            img_slice = zoom(img_slice, zoom_factor, order=1)
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Inference, Evaluate and Visualize PA Refinement')
    ...
    parser.add_argument('--target_class', type=int, default=7, help='Target class index to extract from original mask (e.g., 7 for PA)')

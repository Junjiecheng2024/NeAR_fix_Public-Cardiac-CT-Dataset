"""PyTorch Lightning trainer for PA (stage1) NeAR.
"""
import os
import sys
import argparse
...
from lightning_module import PANeARLightningModule
...
def main(args):
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_PA'
    ...
    pl_module = PANeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project='NeAR_stage1_PA', name=cfg['run_flag'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_PA', help='Config module name (without .py)')

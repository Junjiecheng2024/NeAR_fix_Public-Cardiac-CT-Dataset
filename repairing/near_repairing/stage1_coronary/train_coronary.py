"""
阶段一：冠状动脉（Coronary，类别9）单类精修训练脚本

功能说明：
1. 使用 Shape-only NeAR 隐式表征模型修复冠状动脉的噪声标签
2. 核心策略：
   - 边界偏采样（Boundary-biased sampling）：50%→30%→10% 渐进降低
   - 组合损失函数：70% Dice Loss + 30% Focal Loss (gamma=4.0)
   - 课程学习（Curriculum Learning）：逐步适应真实分布
   - 128³ 高分辨率采样：提供8倍细节以精确捕捉细小结构
3. 训练目标：过拟合以获得最佳精修标签（不考虑泛化，专注label cleaning）
4. 输出：修复后的隐式函数 F_c 和每个样本的潜向量 z_i
...
def train_epoch(model, optimizer, scheduler, scaler, loader, shape_loss_fn, gather_fn,
                ...
                l2_penalty_weight, grad_accum_steps, use_amp, use_cosine_schedule, 
                writer, epoch, iteration):
    ...
    for batch_idx, (indices, shape) in enumerate(loader):
        ...
        if (batch_idx + 1) % grad_accum_steps == 0 or (batch_idx + 1) == len(loader):
            ...
            if use_cosine_schedule:
                scheduler.step()  # Cosine scheduler updates per step
...
def main():
    ...
    if cfg.get("use_cosine_schedule", False):
        ...
        # MultiStepLR scheduler with gamma from config
        gamma = cfg.get("gamma", 0.5)  # Default 0.5 if not specified
        scheduler = torch.optim.lr_scheduler.MultiStepLR(
            optimizer, cfg["milestones"], gamma=gamma
        )
        print(f"Using MultiStepLR scheduler:")
        print(f"  Milestones: {cfg['milestones']}")
        print(f"  Gamma: {gamma} (lr减半)")
    ...
    for epoch in range(start_epoch, cfg["n_epochs"] + 1):
        ...
        # Update scheduler for MultiStepLR (per-epoch)
        if not cfg.get("use_cosine_schedule", False):
            scheduler.step()
        ...
        # Note: For MultiStepLR, scheduler.step() is called per-epoch after train_epoch
        # For Cosine scheduler, scheduler.step() is called per-step inside train_epoch

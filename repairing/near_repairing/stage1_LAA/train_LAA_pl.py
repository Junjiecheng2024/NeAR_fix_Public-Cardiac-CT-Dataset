"""PyTorch Lightning trainer for LAA (stage1) NeAR.
"""
import os
import sys
import argparse
...
from lightning_module import LAANeARLightningModule
...
def main(args):
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_LAA'
    ...
    dm = LAADataModule(cfg)
    dm.setup()

    n_samples = len(dm.train_dataset)
    pl_module = LAANeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project='NeAR_stage1_LAA', name=cfg['run_flag'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_LAA', help='Config module name (without .py)')

"""PyTorch Lightning trainer for LV (stage1) NeAR.
"""
import os
import sys
import argparse
...
from lightning_module import LVNeARLightningModule
...
def main(args):
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_LV'
    ...
    pl_module = LVNeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project='NeAR_stage1_LV', name=cfg['run_flag'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_LV', help='Config module name (without .py)')

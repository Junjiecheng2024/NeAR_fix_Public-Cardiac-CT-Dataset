"""PyTorch Lightning trainer for Aorta (stage1) NeAR.
"""
import os
import sys
import argparse
...
from lightning_module import AortaNeARLightningModule
...
def main(args):
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_Aorta'
    ...
    dm = AortaDataModule(cfg)
    dm.setup()

    n_samples = len(dm.train_dataset)
    pl_module = AortaNeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project='NeAR_stage1_Aorta', name=cfg['run_flag'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_Aorta', help='Config module name (without .py)')

"""PyTorch Lightning trainer for RV (stage1) NeAR.

Usage:
  - For local single-GPU debugging:
      python train_RV_pl.py --devices 1 --config config_RV
  - For multi-GPU (on cluster with 4 A100):
      python train_RV_pl.py --devices 4 --strategy ddp --config config_RV
...
def main(args):
    # Load config
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_RV'
    ...
    pl_module = RVNeARLightningModule(
        n_samples=n_samples,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_RV', help='Config module name (without .py)')

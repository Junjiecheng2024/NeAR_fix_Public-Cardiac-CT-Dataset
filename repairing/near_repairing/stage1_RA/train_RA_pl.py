"""PyTorch Lightning trainer for RA (stage1) NeAR.
"""
import os
import sys
import argparse
...
from lightning_module import RANeARLightningModule
...
def main(args):
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_RA'
    ...
    pl_module = RANeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project='NeAR_stage1_RA', name=cfg['run_flag'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_RA', help='Config module name (without .py)')

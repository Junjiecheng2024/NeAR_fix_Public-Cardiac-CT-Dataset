"""PyTorch Lightning trainer for PV (stage1) NeAR.
"""
import os
import sys
import argparse
...
from lightning_module import PVNeARLightningModule
...
def main(args):
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_PV'
    ...
    pl_module = PVNeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project='NeAR_stage1_PV', name=cfg['run_flag'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_PV', help='Config module name (without .py)')

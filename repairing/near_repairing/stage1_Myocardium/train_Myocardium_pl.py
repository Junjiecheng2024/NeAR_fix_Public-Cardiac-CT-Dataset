"""PyTorch Lightning trainer for Myocardium (stage1) NeAR.
"""
import os
import sys
import argparse
...
from lightning_module import MyocardiumNeARLightningModule
...
def main(args):
    config_name = args.config if hasattr(args, 'config') and args.config else 'config_Myocardium'
    ...
    dm = MyocardiumDataModule(cfg)
    dm.setup()

    n_samples = len(dm.train_dataset)
    pl_module = MyocardiumNeARLightningModule(
        n_samples=n_samples,
    ...
    wandb_logger = WandbLogger(project='NeAR_stage1_Myocardium', name=cfg['run_flag'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', type=str, default='config_Myocardium', help='Config module name (without .py)')
